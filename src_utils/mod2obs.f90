...
!    existing bore sample file.
	...
	integer      :: ifail,ierr,ncol,nrow,i,j,modunit,dds,mms,yys,hhhs,&
			...
			ntrans,ilay,cols,ndays,nsecs,k, &
	...
	write(amessage,5)
5       format(' Program MOD2OBS writes a bore sample file of model-generated ',&
	'heads over time, interpolated to the sites and times ',   &
        'supplied in an existing bore sample file.')
