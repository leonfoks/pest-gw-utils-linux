...
! -- Program PPK2FACM performs the first step in kriging from a set of
!    pilot points to a FEHM finite element mesh; it calculates the
!    factors by which the value assigned to each pilot point contributes to
!    the value at each mesh node. Note that this is done for only one zone
!    based on coordinates for that zone read from a MATXY output file.
        ...
        integer     :: ifail,i,ierr,structunit, &
                       numint,itemp,numstruct,numvario,outunit, &
                       nbb,j,izone,ndat,ngrid,istruct,k_ktype,n_nst,regunit,itrans, &
                       im,femunit,jline,imat,mnode,numnode,ipoint
        ...
        write(amessage,5)
5       format(' Program PPK2FACM calculates point-to-node factors by which kriging ',&
        'is undertaken from a set of pilot points to those elements of a FEHM mesh ',  &
        'representing one zone.')
...
69      write(6,*)
70      aprompt=' Enter MATXY output file containing FEHM node '// &
...
! -- Arrays are allocated and the MATXY output file is read.

        read(femunit,*,err=9000,end=9050) imat,numnode
        allocate(eastgrid(numnode),northgrid(numnode),icellno(numnode),    &
        east(numnode),north(numnode),stat=ierr)
        if(ierr.ne.0) then
          write(amessage,50)
50        format(' Insufficient memory to continue execution.')
        ...
        do
          mnode=mnode+1
          read(femunit,*,err=9020,end=51) east(mnode),north(mnode)
        end do
51      close(unit=femunit)
        mnode=mnode-1
...
! -- zonal arrays are allocated.

        numint=1
        intval(1)=imat
        allocate(astructure(numint),radmax(numint),radmin(numint), &
        akrig(numint),minpt(numint),maxpt(numint),stat=ierr)
        if(ierr.ne.0) then
          write(amessage,50)
          go to 9890
        end if
...
280     format('    Enter structure name: ')
        read(5,'(a)') atemp
        if(index(eschar,atemp(1:2)).ne.0)then
          if(i.gt.2) then
            ...
            deallocate(astructure,radmax,radmin,akrig,minpt,maxpt)
            if(ierr.ne.0) then
              write(amessage,57)
57            format(' Memory management error: cannot continue execution.')
              go to 9890
            end if
            deallocate(eastgrid,northgrid,icellno,east, north,stat=ierr)
            if(ierr.ne.0)then
              write(amessage,57)
              go to 9890
            end if
...
!    the finite-difference grid using GSLIB subroutines.
        ...
        if(aoutfile.eq.'f')then
          write(outunit,'(a)') trim(femfile)
        else
          write(outunit) femfile
        end if
        ...
        do izone=1,numint
          ...
          itemp=intval(izone)
          do im=1,ngrid
                eastgrid(im)=east(im)-east1
                northgrid(im)=north(im)-north1
                icellno(im)=im
          end do
          ndat=0
          do i=1,num_pilot_points
...
9010    format(' Error encountered while reading MATXY output file ',a)
        go to 9890
9020    call num2char(mnode+1,aline)
        write(amessage,9030) trim(aline),trim(femfile)
9030    format(' Error encountered while reading line ',a,' of MATXY ', &
        'output file ',a,'.')
        go to 9890
9050    write(amessage,9060) trim(femfile)
9060    format(' Unexpected end encountered to MATXY output file ',a)
        go to 9890
...
9900    call close_files
        ...
        call free_point_mem()
        deallocate(east,north,eastgrid,northgrid,icellno,stat=ierr)
