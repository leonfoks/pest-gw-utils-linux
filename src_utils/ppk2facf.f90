!     Last change:  JD    9 May 2003   11:41 am

program ppk2facf

! -- Program PPK2FACF performs the first step in kriging from a set of
!    pilot points to a MicroFEM finite element mesh; it calculates the
...
!    the value at each mesh node.
        ...
        integer     :: ifail,i,ierr,structunit, &
                       ...
                       im,nummesh,labunit,femunit,jline
        real        :: s_skmean,c_c0,pmx
        double precision :: minsep,minsep2,eastdiff,northdiff,distance,dtemp,east1, &
                       north1,etemp,ntemp
        integer     :: iloc(1)
        ...
        write(amessage,5)
5       format(' Program PPK2FACF calculates point-to-node factors by which kriging ',&
        'is undertaken from a set of pilot points to a MicroFEM mesh.')
...
69      write(6,*)
70      aprompt=' Enter name of fem file defining finite element mesh: '
        call open_input_file(ifail,aprompt,femfile,femunit)
        if(ifail.ne.0) go to 9900
        if(escset.eq.1) then
          escset=0
          write(6,*)
...
75      aprompt=' Enter name of special ASCII label file defining zonation: '
        call open_input_file(ifail,aprompt,labfile,labunit)
        if(ifail.ne.0) go to 9900
        if(escset.eq.1) then
          escset=0
          close(unit=femunit)
...
! -- The fem file is read a little in order to ascertain the number of nodes in
!    the mesh.

        do i=1,15
          read(femunit,*,err= 9000,end=9050)
        end do
        read(femunit,*,err=9000,end=9050) nummesh
        close(unit=femunit)
        allocate(east(nummesh),north(nummesh),eastgrid(nummesh),northgrid(nummesh), &
        icellno(nummesh),intarray(nummesh),stat=ierr)
        if(ierr.ne.0) then
          write(amessage,50)
50        format(' Insufficient memory to continue execution.')
          go to 9890
        end if

! -- Grid coordinates and zone numbers are read from the special label file. Note that
! -- the coordinates of the first grid node are subtracted from all mesh coordinates.

        jline=0
        numint=0
        currint=-99999
        jline=jline+1
52      read(labunit,'(a)',err=9100,end=9150) cline
        if(cline.eq.' ') go to 52
        call linesplit(ifail,3)
        if(ifail.ne.0) go to 9500
        east1=char2double(ifail,1)
        if(ifail.ne.0) go to 9550
        north1=char2double(ifail,2)
        if(ifail.ne.0) go to 9550
        intarray(1)=char2int(ifail,3)
        if(ifail.ne.0) go to 9550
        east(1)=0.0
        north(1)=0.0
        do i=2,nummesh
          jline=jline+1
53        read(labunit,'(a)',err=9100,end=9150) cline
          if(cline.eq.' ') go to 53
          call linesplit(ifail,3)
          if(ifail.ne.0) go to 9500
          etemp=char2double(ifail,1)
          if(ifail.ne.0) go to 9550
          ntemp=char2double(ifail,2)
          if(ifail.ne.0) go to 9550
          intarray(i)=char2int(ifail,3)
          if(ifail.ne.0) go to 9550
          east(i)=etemp-east1
          north(i)=ntemp-north1
        end do
        close(unit=labunit)

! -- The coordinates of first mesh node are subtracted from all pilot point coords.
...
! -- The zone numbers are now collected and counted.

        numint=1
        currint=intarray(1)
        intval(1)=currint
        mesh_travel: do i=1,nummesh
          newint=intarray(i)
          if(newint.eq.currint) cycle
          currint=newint
          prev_int: do j=1,numint
              if(newint.eq.intval(j)) cycle mesh_travel
          end do prev_int
          numint=numint+1
          if(numint.gt.MAXINT) then
            call num2char(MAXINT,anum1)
            write(amessage,150) trim(anum1),trim(labfile)
150         format(' The special ASCII label file can hold a maximum of ',a, &
            ' different integers for proper PPK2FACF execution. There are ', &
            ' more integers than this in file ',a,'. To increase ' &
            'this limit, edit PPK2FACF source code, increase parameter ', &
...
200     write(6,*)
205     write(6,210)
210     format(' The following zones have been detected in the special ASCII ',&
        'label file:-')
        i=0
255        i=i+1
...
280     format('    Enter structure name (blank if no interpolation for this zone): ')
        ...
        if(index(eschar,atemp(1:2)).ne.0)then
          if(i.gt.2) then
            ...
            deallocate(astructure,radmax,radmin,akrig,minpt,maxpt,intarray,   &
            east,north,eastgrid,northgrid,icellno,stat=ierr)
            if(ierr.ne.0) then
              write(amessage,57)
...
350     continue
        ...
        write(amessage,352)
352     format(' Interpolation must take place for at least one zone in mesh ', &
        'property file - try again.')
        call write_message(leadspace='yes',endspace='yes')
        go to 200
351     continue
...
!    the finite-difference grid using GSLIB subroutines.
        ...
        do izone=1,numint
          ...
          itemp=intval(izone)
          do im=1,nummesh
              if(intarray(im).eq.itemp)then
                ngrid=ngrid+1
                eastgrid(ngrid)=east(im)
                northgrid(ngrid)=north(im)
                icellno(ngrid)=im
              end if
          end do
...
399         format(/,' Warning: no pilot points assigned to mesh zone ',a)
            go to 410
          end if
          call num2char(intval(izone),anum1)
          write(6,401) trim(anum1)
401       format(/,' Carrying out interpolation for mesh zone ',a,'....')
          do i=1,numstruct
...
422           format(' Variogram "',a,'" uses the power model. Simple kriging ', &
          ...
          outfile,aoutfile,outunit,pmx,itrans,num_pilot_points,regarray)
...
450     format('  - kriging factors written to file ',a)
        ...
        do i=1,num_pilot_points
          write(regunit,'(a)') trim(pilot_point_id(i))
        end do
        do i=1,num_pilot_points
          write(regunit,460) (regarray(j,i),j=1,num_pilot_points)
...
9000    write(amessage,9010) trim(femfile)
9010    format(' Error encountered while reading MICROFEM "fem" file ',a)
        go to 9890
9050    write(amessage,9060) trim(femfile)
9060    format(' Unexpected end encountered to MICROFEM "fem" file ',a)
        go to 9890
9100    write(amessage,9110) trim(labfile)
9110    format(' Error encountered while reading MICROFEM label file ',a)
        go to 9890
9150    write(amessage,9160) trim(labfile)
9160    format(' Unexpected end encountered to MICROFEM label file ',a)
        go to 9890
9500    call num2char(jline,aline)
        write(amessage,9510) trim(aline),trim(labfile)
9510    format(' Insufficient entries on line ',a,' of file ',a)
        go to 9890
9550    call num2char(jline,aline)
        write(amessage,9560) trim(aline),trim(labfile)
9560    format(' Error reading data from line ',a,' of file ',a)
        go to 9890
...
9900    call close_files
        ...
        call free_point_mem()
        deallocate(intarray,east,north,eastgrid,northgrid,icellno,stat=ierr)
...
!              '   standard deviation  = ',f12.5,/)
      ...
      aoutfile,outunit,pmx,itrans,npp,regarray)
