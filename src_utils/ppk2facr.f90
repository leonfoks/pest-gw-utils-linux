...
! -- Program PPK2FACR performs the first step in kriging from a set of
!    pilot points to a two-dimensional RSM mesh. Use of these factors is
!    made by the FAC2RSM utility which performs the actual spatial interpolation.
        ...
        write(amessage,5)
5       format(' Program PPK2FACR calculates point-to-mesh-centroid factors by which kriging ',&
       'is undertaken from a set of pilot points to an RSM mesh.')
...
! -- Memory is allocated.
        ...
        allocate(idata(numelem_g),stat=ierr)
...
30      call read_pilot_points_file(ifail, &
        ...
        if(escset.ne.0) then
          escset=0
          deallocate(eastnode_g,northnode_g,nodenum_g,idata,elemnum_g,elemindex_g,elemnode_g,  &
...
!        end if
        ...
        write(6,*)
68      write(6,67,advance='no')
67      format(' Read zonal index file? (y/n): ')
        read(5,'(a)') aa
        if(aa.eq.' ') go to 68
        ...
        if((aa.ne.'y').and.(aa.ne.'n')) go to 68

71      continue
        usezones=0
        if(aa.eq.'y')then
          aprompt=' Enter name of zonal index file: '
          call read_gms_integer_mesh_data_file(ifail,aprompt,idata)
          if(ifail.ne.0) go to 9900
          if(escset.eq.1) then
            escset=0
            write(6,*)
...
150         format(' The index integer file can hold a maximum of ',a, &
            ' different integers for proper PPK2FACR execution. The data ', &
            'contained in file ',a,' cites more than this. To increase ',&
            'this limit, edit PPK2FACR source code, increase parameter ', &
...
200       write(6,*)
205       write(6,210)
210       format(' The following zones have been detected in the index data ',&
          'file:-')
        else
          ispace=1
        end if
...
281       format(' Enter structure name for calculation of kriging factors: ')
        end if
        read(5,'(a)') atemp
...
!    is calculated for every centroid in the mesh.
        ...
        east1=eastnode_g(1)
        north1=northnode_g(1)
        do i=1,numelem_g
          eastelem_g(i)=eastelem_g(i)-east1
          northelem_g(i)=northelem_g(i)-north1
        end do
        ...
        if(ierr.ne.0) go to 9200
        allocate(eastgrid(numelem_g),northgrid(numelem_g),icellno(numelem_g),stat=ierr)
        if(ierr.ne.0) go to 9200
...
!    the finite-difference grid using GSLIB subroutines.
        ...
        do izone=1,numint
          ...
          itemp=intval(izone)
          do i=1,numelem_g
            if(idata(i).eq.itemp)then
              ngrid=ngrid+1
              eastgrid(ngrid)=eastelem_g(i)
              northgrid(ngrid)=northelem_g(i)
              call whichone_i(ifail,numelem_g,jj,elemindex_g,i)
              icellno(ngrid)=jj
            end if
          end do
...
399         format(/,' Warning: no pilot points assigned to integer zone ',a,'.')
            go to 410
          end if
          if(usezones.eq.1)then
            call num2char(intval(izone),anum1)
            write(6,401) trim(anum1)
401         format(/,' Carrying out interpolation for integer zone ',a,'....')
          else
            write(6,403)
403         format(/,' Carrying out interpolation....')
          end if
          do i=1,numstruct
...
9900    call close_files
        call free_point_mem()
        deallocate(eastgrid,northgrid,icellno,idata,stat=ierr)
...
!
 4    continue
      if(nk.ge.1) then
            ...
            vk = vk/real(nk) - ak*ak
             write(6,105) nk
105          format('   No. of element centroids for which factors were calculated = ',i5)
!             write(6,106) ak
