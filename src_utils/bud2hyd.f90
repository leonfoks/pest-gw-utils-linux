!     Last change:  JD   12 Apr 2003    2:27 pm
program bud2hyd

! -- Program BUD2HYD writes a time series file of model-generated system
!    inflows/outflows within user-specified zones.
	...
	allocate(flowrate(MAXZONE),jzone(MAXZONE),stat=ierr)
	if(ierr.ne.0) go to 9500
	flowrate=0
	jzone=0

	write(amessage,5)
5       format(' Program BUD2HYD writes a time series file of ',             &
...
66	format(' Is this a MODFLOW88 or MODFLOW96 budget file  [8/9]? ',$)
	...
	if(am.eq.'8')then
	  write(amessage,72)
72	  format(' BUD2HYD cannot continue execution as it can read ',   &
...
260	format('  - a total of ',a,' different non-zero zones identified ',  &
        'in integer arrays.')
	call write_message()

300	write(6,*)
310	call open_output_file(ifail, &
	' Enter name for time series output file: ',outfle,outunit)
	if(ifail.ne.0) go to 9900
	if(escset.ne.0) then
	  escset=0
	  write(6,*)
...
397	format(' Stress_period  Time_step  Elapsed_time    Flow_type     ',   &
	'     Flow_processed_by_BUD2HYD')

	write(outunit,400,advance='no')
400	format(t2,'Flow_type',t21,'Elapsed_Time',t37,'Date',t49,'Time    ')
	do izone=1,numzone
	  write(atemp,'(i4)') jzone(izone)
	  atemp=adjustl(atemp)
	  write(outunit,420,advance='no') 'Flow_rate_'//trim(atemp)
420	  format(a16)
	end do
	write(outunit,*)
...
800	itime=0
	read_an_array: do
	  ...
	  if(mlay.gt.0)then
!	    write(6,*) kstp,kper,text,mcol,mrow,mlay
	        ...
	        do irow=1,nrow
	          do icol=1,ncol
	            if(iarray(icol,irow,ilay).eq.itemp)then
	              flowrate(izone)=flowrate(izone)+budarray(icol,irow,ilay)
	            end if
	          end do
	        end do
	      end do
...
!	      write(6,*) icol,irow,ilay
	      ...
	      do izone=1,numzone
	        if(jzone(izone).eq.itemp) then
	          flowrate(izone)=flowrate(izone)+q
	          go to 780
	        end if
	      end do
780	      continue
	      ...
	      do irow=1,nrow
	        do icol=1,ncol
	          if(iarray(icol,irow,1).eq.itemp)then
	            flowrate(izone)=flowrate(izone)+budarray(icol,irow,1)
	          end if
	        end do
	      end do
	    end do
...
782         format(' Unknown compact storage method: data type = ',a, &
	  ...
	  write(outunit,430) trim(text),timwrit,adate,atime,                           &
          (flowrate(izone)*fac,izone=1,numzone)
430	  format(1x,a,t20,1pe13.6,t36,a10,t48,a10,2x,1000(1x,1pg15.8))
	  itime=itime+1
...
9900	call close_files
	call free_grid_mem(gridspec)
	deallocate(budarray,iarray,flowrate,intarray,stat=ierr)
	write(6,*)
