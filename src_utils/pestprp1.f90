...
! -- Program PESTPRP1 prepares a PEST control file and a PEST instruction
!    file using pre-existing template files as well as a bore sample file
!    written by MOD2OBS or SMP2SMP.
...
!       May, 1998: verison 1.
	...
	integer	:: ifail,sampunit1,sampunit2,iobs,nobs,maxobs,iline,idate, &
	           ...
	           j1,j2,ipar,ll,jj,kk,dd,ss,iichar,npargp,outunit
	integer :: numpargp(27)
	integer, allocatable    :: numsamp(:),bore_index(:),ndays(:),nsecs(:)
	real, allocatable       :: value(:)
	double precision        :: dtemp
	character (len=80)	:: sampfile1,sampfile2,specfile,insfle,outfle
	character (len=80), allocatable	:: tempfle(:)
	...
	character (len=8)       :: aachar,aobs,atemp,aapar,apar(MAXPAR)
	...
	write(amessage,5)
5       format(' Program PESTPRP prepares a PEST control file and ', &
	'PEST instruction files based on pre-existing template files and ',&
	'a bore sample file written by MOD2OBS or SMP2SMP.')
...
90	    format(' Line ',a,' of MOD2OBS-generated bore sample file ', &
...
270	    format(' Line ',a,' of MOD2OBS-generated bore sample file ', &
...
280	    format(' Line ',a,' of MOD2OBS-generated bore sample file ', &
...
510	format(' How many template files are there? ')
	...
	allocate(tempfle(numtemp),stat=ierr)
	if(ierr.ne.0) go to 9100
	i=0
	npar=0
...
566	    format(' Blank parameter space at line ',a,' of template ', &
	    ...
	    go to 9890
567	    continue
	    if(kk-jj+1.gt.8)then
	      call num2char(iline,aline)
	      write(amessage,568) trim(aline),trim(tempfle(i))
568	      format(' Parameter name greater than 8 characters in ',  &
...
635	          format(' Samples in file ',a,' are in a different order ', &
	          ...
	          'latter was built from the former using MOD2OBS.')
	          go to 9890
	        end if
	      end if
	      go to 640
...
680	format(' This would not have happened if file ',a,' were built from ', &
	'file ',a,' using MOD2OBS as it should have been.')
...
740	format('restart')
!	write(outunit,750,err=9500) npar,nobs,npargp,0,1  used in PESTPREP
	write(outunit,750,err=9500) npar,nobs,npar,0,1
750	format(1x,5i6)
	write(outunit,760,err=9500) numtemp,1
760	format(1x,i5,1x,i5,' single  point')
...
800	format(' 30  0.01  3  3  0.01  3')
	write(outunit,810,err=9500)
810	format(' 1  1  1')
	write(outunit,820,err=9500)
820	format('* parameter groups')

! -- The following code from PESTPREP is commented out and replaced with code 
!    for PESTPRP1

	do i=1,npar
	      write(outunit,840,err=9500) apar(i)
840	      format(1x,a8,5x,'relative  ',2x,'0.02  0.0  switch  2.0 parabolic')
	end do
...
!870	    format(1x,a8,1x,'log  factor   1.0  1.0e-10  1.0e10 ',a,'     1.0  0.0')
!	  else if(achar.eq.'s')then
!	    write(outunit,875,err=9500) apar(i),achar
!875	    format(1x,a8,1x,'log  factor   0.1  1.0e-10  0.3    ',a,'     1.0  0.0')
!	  else if((iichar.ge.97).and.(iichar.le.122))then
!	    write(outunit,880,err=9500) apar(i),achar
	    write(outunit,880,err=9500) apar(i),apar(i)
880	    format(1x,a8,1x,'none relative 1.0  -1.0e10  1.0e10 ',a8,'     1.0  0.0')
!880	    format(1x,a8,1x,'none relative 1.0  -1.0e10  1.0e10 ',a,'     1.0  0.0')
!	  else
!	    write(outunit,885,err=9500) apar(i)
!885	    format(1x,a8,1x,'none relative 1.0  -1.0e10  1.0e10 other 1.0  0.0')
!	  end if
	end do
...
920	    format(1x,a,t10,1pg14.7,2x,'1.0  obsgroup')
          end do
        end do
	write(outunit,930,err=9500)
930	format('* model command line')
	write(outunit,940,err=9500)
940	format(' model')
	write(outunit,950,err=9500)
950	format('* model input/output')
	do i=1,numtemp
	  call num2char(i,aline)
	  write(outunit,960,err=9500) trim(tempfle(i)),trim(aline)
960	  format(1x,a,2x,'model',a,'.in')
	end do
	write(outunit,970,err=9500) trim(insfle),trim(sampfile2)
...
9900    call close_files
	...
	tempfle,value,stat=ierr)
	write(6,*)

end program pestprp1
