...
!       accept_blank:  "yes" if a blank response is accepted as indicating no file
	...
	if(associated(pilot_point_val1)) deallocate (pilot_point_val1)
	nullify(pilot_point_id,pilot_point_east,pilot_point_north,pilot_point_zone, &
        pilot_point_val,pilot_point_val1)
...
220     format(' Working.....')
	...
	pilot_point_north(npoint),pilot_point_zone(npoint), &
        pilot_point_val(npoint),pilot_point_val1(npoint),stat=ierr)
	if(ierr.ne.0) then
	  write(amessage,250)
...
395         format(1x,a,'cannot read first value assigned to point.')
	    call write_message(increment=1)
	  end if
	  pilot_point_val1(num_pilot_points)=char2double(ifail,6)
	  if(ifail.ne.0)then
	    if(imessage.eq.0) call write_initial_message(leadspace='yes')
	    write(amessage,396) amsg(1:imsg)
396         format(1x,a,'cannot read second value assigned to point.')
	    call write_message(increment=1)
	  end if
...
370       format('  - data for ',a,' pilot points read from pilot ',&
	  'points file ',a)
	  call write_message
	  ifail=0
	  go to 9999
