...
!  -- Subroutine read_int_tab_file reads an integer array table file, checking
!     it for errors and transferring the results to an integer array.

! -- Arguments are as follows:-
!       ifail:     returned as non-zero if error condition encountered.
!       tabunit:   unit number of integer array table file
!       tabfile:   integer array table fileneme
...
!       June-November, 1995: version 1.
	...
	integer                                 :: icol,irow,ival,iline
	character (len=10)                      :: aline

	write(initial_message,20) trim(tabfile)
20      format(' Errors in integer array table file ',a,':-')
...
160         format('   Line ',a,': grid column number exceeds grid column ',&
	...
	ival=char2int(ifail,3)
	if(ifail.ne.0) then
	  if(imessage.eq.0) call write_initial_message(leadspace='yes')
	  call num2char(iline,aline)
	  write(amessage,180) trim(aline)
180       format('   Line ',a,': cannot read integer array value from ',&
	  'third column.')
	  call write_message(increment=1)
	else
	  if((ival.lt.-999).or.(ival.gt.9999)) then
	    if(imessage.eq.0) call write_initial_message(leadspace='yes')
	    call num2char(iline,aline)
	    write(amessage,200) trim(aline)
200         format('   Line ',a,': integer array value must lie between ', &
	    '-999 and 9999')
	    call write_message(increment=1)
	  else
	    if(imessage.eq.0) intarray(icol,irow)=ival
	  end if
	end if
	go to 30
...
1030      format(' - ',a,' lines read from integer array table file ',a)
...
9010    format(' Cannot read line ',a,' of integer array table file ',a)
	call write_message(leadspace='yes',endspace='yes')
	ifail=1
	return
