...
! -- Subroutine READ_GMS_2D_MESH_DATA_FILE reads a GMS mesh data file in which
...
! -- The name of the GMS integer data file is acquired.
...
! -- The GMS integer data file is opened.
...
! -- The integer data is now searched for.
...
51        continue
          ...
          call linesplit(ifail,1)
          rtemp=char2real(ifail,1)
          if(ifail.ne.0)then
            if(imessage.eq.0) call write_initial_message(leadspace='yes')
            call num2char(iline,aline)
            write(amessage,60) trim(aline),trim(afile)
60          format(' Error reading number from line ',a,' of file ',a,'.')
            call write_message(increment=1)
            cycle
          end if
          iitemp=nint(rtemp)
          if(iitemp.eq.0)then
            if(rtemp.ne.0.0) go to 70
          else
            if(abs(float(iitemp)-rtemp).gt.abs(float(iitemp)*1.0e-5)) go to 70
          end if
          intdata(elemindex_g(i))=iitemp
69        continue
          go to 90
70        call num2char(iline,aline)
          write(amessage,80) trim(aline),trim(afile)
          if(imessage.eq.0) call write_initial_message(leadspace='yes')
80        format(' Number read from line ',a,' of file ',a,' is not an integer.')
          call write_message(increment=1)
90        continue
        end do
...
!          do i=1,numelem_g                       !debug
!            write(99,*) elemindex_g(i)             !debug
!          end do                                 !debug
!          write(99,*)                             !debug
!          do i=1,numelem_g                       !debug
!            write(99,*) intdata(i)               !debug
!          end do                                 !debug
!          close(unit=99)                         !debug
          go to 9900
        end if
