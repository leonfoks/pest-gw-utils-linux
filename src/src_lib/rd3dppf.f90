...
! -- Subroutine read_3d_pilot_points_file reads a 3d pilot points file, checking
! -- it for errors.
...
!       accept_blank:  "yes" if a blank response is accepted as indicating no file
	...
	if(associated(pilot_point_elev)) deallocate(pilot_point_elev)
	...
	nullify(pilot_point_id,pilot_point_east,pilot_point_north,pilot_point_zone, &
        pilot_point_val,pilot_point_elev)
...
20        format('cannot open 3D pilot points file ',a,' - try again.')
...
30      format(' Errors in 3D pilot points file ',a,' ----->')
...
70          format('   Line ',a,': insufficient entries (six ', &
	    'entries required).')
...
!220     format(' Working.....')
	...
	pilot_point_north(npoint),pilot_point_zone(npoint), &
        pilot_point_val(npoint),pilot_point_elev(npoint),stat=ierr)
	if(ierr.ne.0) then
	  write(amessage,250)
...
350         format(1x,a,'cannot read point north coordinate.')
	  ...
	  pilot_point_elev(num_pilot_points)=char2double(ifail,4)
	  if(ifail.ne.0)then
	    if(imessage.eq.0) call write_initial_message(leadspace='yes')
	    write(amessage,352) amsg(1:imsg)
352         format(1x,a,'cannot read point elevation.')
	    call write_message(increment=1)
	  end if
	  pilot_point_zone(num_pilot_points)=char2int(ifail,5)
	  if(ifail.ne.0) then
	    if(imessage.eq.0) call write_initial_message (leadspace='yes')
...
!	    end if
	  ...
	  pilot_point_val(num_pilot_points)=char2double(ifail,6)
	  if(ifail.ne.0)then
	    if(imessage.eq.0) call write_initial_message(leadspace='yes')
