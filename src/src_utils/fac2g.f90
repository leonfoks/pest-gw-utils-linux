...
! -- Program FAC2G writes mesh data to a general nodal value file based on the
!    interpolation factor file produced by PPK2FACG.
	...
	write(amessage,5)
5	format(' Program FAC2G carries out spatial interpolation from pilot points ',  &
        'to a set of grid/mesh nodes based on interpolation factors calculated by PPK2FACG.')
...
10      aprompt=' Enter name of interpolation factor file: '
	...
	if(ierr.ne.0) then
	  write(amessage,50)
50	  format(' Cannot allocate sufficient memory to run FAC2G.')
	  go to 9890
	end if
...
59            format(' The pilot points in the pilot points file are not the same, ', &
              ...
              'pilot points file read by PPK2FACG when it calculated the ', &
...
330       format(' Upper interpolation limit does not exceed lower interpolation ', &
        ...
        write(6,*)
1095    aprompt = ' Enter name for output file: '
...
! -- The output file is written.
        ...
        do i=1,nummesh
          write(propunit2,199) realarray(i)
199       format(1x,1pg14.7)
        end do
