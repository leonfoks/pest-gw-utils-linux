...
! -- Program PPK2FAC3 performs the first step in kriging from a set of
...
!    Certain features of its functionality were designed for use with long and
!    stringy alluvial valleys.
	...
	implicit none
        ...
        integer        numex,eunit,iline,ns,intnumber,inum,iz,k,izz,ppz
        real        :: s_skmean,c_c0,pmx
        real        :: blankrad
        real        :: fracreject,distint,absreject
	...
	integer, allocatable, dimension(:,:)           :: ezone(:,:)
	integer, allocatable, dimension(:,:)           :: iwarray(:,:)
	...
	write(amessage,5)
5	format(' Program PPK2FAC3 calculates point-to-cell factors by which kriging ',&
...
10      call spec_open(ifail,gridspec)
	...
	ncol=gridspec%ncol
	nrow=gridspec%nrow
	allocate(intarray(ncol,nrow),iwarray(ncol,nrow),stat=ierr)
	if(ierr.ne.0) then
	  write(amessage,50)
50	  format(' Cannot allocate sufficient memory to run PPK2FAC3.')
	  go to 9890
	end if
	iwarray=1        ! an array
...
500     continue
        ...
        atempf=' '
        numex=0
        write(6,510,advance='no')
510     format(' Enter name of exclusion zone file (<Enter> if none): ')
        read(5,'(a)') atempf
        if(atempf.ne.' ')then
          ...
          nbb=len_trim(atempf)
          call getfile(ifail,atempf,efile,1,nbb)
          if(ifail.ne.0) go to 500
          eunit=nextunit()
          open(unit=eunit,file=efile,status='old',iostat=ierr)
          if(ierr.ne.0)then
            write(amessage,530) trim(efile)
530         format(' Cannot open file ',a,'  - try again.')
            call write_message()
            go to 500
          end if
          allocate(ezone(numint-1,numint),stat=ierr)
          if(ierr.ne.0)then
            write(amessage,560)
560         format(' Cannot allocate sufficient memory to continue execution.')
            go to 9890
          end if
          ezone=huge(i)                       ! an array
          iline=0
          do
            iline=iline+1
            call num2char(iline,aline)
            read(eunit,'(a)',end=540) cline
            cline=adjustl(cline)
            if(cline.eq.' ')cycle
            ...
            if(ifail.ne.0)then
              write(amessage,570) trim(aline),trim(efile)
570           format(' At least two entries are expected on line ',a,' of file ',a,'.')
              go to 9890
            end if
            intnumber=1
            inum=char2int(ifail,1)
            if(ifail.ne.0) go to 9000
            do i=1,numint
              if(intval(i).eq.inum) go to 590
            end do
            call num2char(inum,anum)
            go to 9100
590         iz=i
            if(ezone(1,iz).ne.huge(i))then
              call num2char(inum,anum)
              write(amessage,595) trim(anum),trim(aline),trim(efile)
595           format(' Integer zone ',a,' defined as first integer on line ',a,' of file ',a,   &
              ' has already been cited as a target zone.')
              go to 9890
            end if
            if(astructure(iz).eq.' ') go to 623
            numex=numex+1
            do
              intnumber=intnumber+1
              cline=cline(right_word(1)+1:)
              call linesplit(ifail,1)
              if(ifail.ne.0) go to 621
              inum=char2int(ifail,1)
              if(ifail.ne.0) go to 9000
              do i=1,numint
                if(intval(i).eq.inum) go to 600
              end do
              call num2char(inum,anum)
              go to 9100
600           continue
              izz=i
              if(inum.eq.intval(iz))then
                call num2char(inum,anum)
                write(amessage,610) trim(anum),trim(aline),trim(efile)
610             format(' Exclusion zone ',a,' is also cited as target zone at line ',a,' of file ',a,'.')
                go to 9890
              end if
              if(intnumber.gt.2)then
                do i=1,intnumber-2
                  if(izz.eq.ezone(i,iz))then
                    call num2char(inum,anum)
                    write(amessage,620) trim(anum),trim(aline),trim(efile)
620                 format(' Exclusion zone ',a,' cited more than once at line ',a,' of file ',a,'.')
                    go to 9890
                  end if
                end do
              end if
              ezone(intnumber-1,iz)=izz
            end do
621         continue
            do j=1,numint
              if(j.eq.iz) go to 6221
              if(astructure(j).eq.' ') go to 6221
              do k=1,numint-1
                if(j.eq.ezone(k,iz)) go to 6221
              end do
              if(astructure(j).ne.astructure(iz))then
                write(amessage,622) trim(aline),trim(efile)
622             format(' Error on line ',a,' of exlusion file ',a,': all non-excluded zones ',  &
                'must use same geostatistical structure for interpolation as target zone.')
                go to 9890
              end if
6221          continue
            end do
...
540       continue
          if(numex.ne.0)then
            do i=1,numint
              if(astructure(i).eq.' ') cycle
              if(ezone(1,i).eq.huge(itemp))then
                do j=1,numint
                  if(astructure (j).eq.' ') go to 538
                  if(astructure (j).ne.astructure(i))then
                    call num2char(intval(i),anum)
                    call num2char(intval(j),aline)
                    write(amessage,539) trim(anum),trim(aline)
539                 format(' Zone characterized by integer ',a,' is not cited as a target zone in exclusion ',  &
                    'zone file. Therefore interpolation will take place from pilot points in all other zones to this ',  &
                    'zone. However the zone characterized by integer ',a,' has been assigned ', &
                    'a different geostatistical structure to this zone.')
                    go to 9890
                  end if
538               continue
                end do
          ...
          if(numex.eq.0)then
            write(amessage,541) trim(efile)
541         format(' No target or exclusion zones cited in file ',a,'.')
            go to 9890
          else
            call num2char(numex,anum)
            write(6,542) trim(anum),trim(efile)
542         format(' - exclusion zones for ',a,' target zones read from file ',a)
          end if
          close(unit=eunit)
        end if
...
710     format(' Prevent interpolation across inactive areas? [y/n]: ')
        read(5,'(a)') ai
        call casetrans(ai,'lo')
        if(ai.eq.'e')then
          if(allocated(ezone))then
            deallocate(ezone,stat=ierr)
            if(ierr.ne.0)then
              write(amessage,3551)
3551          format(' Memory management error encountered.')
              go to 9890
            end if
          end if
          write(6,*)
          go to 500
        else if((ai.eq.'Y').or.(ai.eq.'y'))then
          ai='y'
        else if((ai.eq.'N').or.(ai.eq.'n'))then
          ai='n'
        else
          go to 700
        end if
...
730       format(' Enter min relative contiguous inactive separation for rejection (0 to 1): ')
          itemp=key_read(fracreject)
          if(escset.eq.1) then
            escset=0
            write(6,*)
            go to 700
          ...
          if((fracreject.lt.0.0).or.(fracreject.gt.1.0))then
            write(6,740)
740         format('   This must be between zero and one - try again.')
            go to 720
          end if
        end if
748     continue
        if(ai.eq.'y')then
749       write(6,750,advance='no')
750       format(' Enter min absolute contiguous inactive separation for rejection: ')
          itemp=key_read(absreject)
          if(escset.eq.1) then
            escset=0
            write(6,*)
            go to 719
          ...
          if(absreject.lt.0.0)then
            write(6,760)
760         format('   This must be greater than zero - try again.')
            go to 749
          end if
        end if
769     continue
...
780       format(' Reject if one or both thresholds is exceeded?  [o/b]: ')
          read(5,'(a)') abo
          call casetrans(abo,'lo')
          if(abo.eq.'e')then
            write(6,*)
            go to 748
          else if((abo.eq.'O').or.(abo.eq.'o'))then
            abo='o'
          else if((abo.eq.'B').or.(abo.eq.'b'))then
            abo='b'
          else
            go to 770
          end if
...
! -- Some calculations are performed pertaining to inactive bridging prevention.

       allocate(delrsum(-1:ncol+1),delcsum(-1:nrow+1),stat=ierr)
       if(ierr.ne.0)then
         write(amessage,560)
         go to 9890
       end if
       delrsum(-1)=-huge(dtemp)
       delrsum(0)=0.0d0
       delrsum(ncol+1)=huge(dtemp)
       do icol=1,ncol
         delrsum(icol)=delrsum(icol-1)+gridspec%delr(icol)
       end do
       delcsum(-1)=huge(dtemp)
       delcsum(0)=0.0d0
       delcsum(nrow+1)=-huge(dtemp)
       do irow=1,nrow
         delcsum(irow)=delcsum(irow-1)-gridspec%delc(irow)
       end do
       distint=huge(distint)
       do irow=1,nrow
         if(gridspec%delc(irow).lt.distint)distint=gridspec%delc(irow)
       end do
       do icol=1,ncol
         if(gridspec%delr(icol).lt.distint)distint=gridspec%delr(icol)
       end do
       distint=distint*0.1            ! arbitrary - make larger for greater speed
        ...
        do izone=1,numint
          ...
          do i=1,num_pilot_points
            if(pilot_point_zone(i).eq.itemp)then
              ...
              if(numex.gt.0)then
                ppz=pilot_point_zone(i)
                do k=1,numint
                  if(intval(k).eq.ppz) go to 649
                end do
                write(amessage,648)
648             format(' Programming error - contact programmer.')
                go to 9890
649             continue
                if(astructure(k).ne.' ')then
                  do iz=1,numint-1
                    if(ppz.eq.intval(ezone(iz,izone))) go to 650
                  end do
                  ndat=ndat+1
                  eastdat(ndat)=pilot_point_east(i)
                  northdat(ndat)=pilot_point_north(i)
...
399         format(/,' Warning: no pilot points assigned to integer array zone ',a,   &
            ' or any non-excluded zone for this target zone.')
            go to 410
          end if
          call num2char(intval(izone),anum1)
...
! -- If inactive bridging is prevented we built an integer array which is zero except
!    for non-excluded zones.
          ...
          if(ai.eq.'y')then
            if(numex.eq.0)then
              ...
              do irow=1,nrow
                do icol=1,ncol
                  if(intarray(icol,irow).eq.itemp) iwarray(icol,irow)=1
                end do
              end do
            else
              iwarray=1         ! an array
              do i=1,numint
                if(astructure(i).eq.' ')then
                  ...
                  do irow=1,nrow
                    do icol=1,ncol
                      if(intarray(icol,irow).eq.itemp) iwarray(icol,irow)=0
                    end do
                  end do
                end if
              end do
          ...
          blankrad,ai,fracreject,absreject,abo,gridspec%cosang,  &
          gridspec%sinang,distint,delrsum,delcsum,iwarray)
...
9000    continue
        call num2char(intnumber,anumber)
        write(amessage,9010) trim(anumber),trim(aline),trim(efile)
9010    format(' Cannot read integer number ',a,' from line ',a,' of file ',a,'.')
        go to 9890

9100    write(amessage,9110) trim(anum),trim(aline),trim(efile)
9110    format(' Integer "',a,'" cited on line ',a,' of file ',a,' is not a zone ', &
        'number featured in zonal integer array file.')
        go to 9890
...
9900    call close_files
	call free_grid_mem(gridspec)
        ...
        deallocate(ezone,stat=ierr)
        deallocate(delrsum,delcsum,stat=ierr)
        deallocate(iwarray,stat=ierr)

end program ppk2fac3
...
!
       ...
       write(6,*)
       write(6,900) nd
900    format('   No. of pilot points used in interpolating to this zone     = ',i5)
       write(6,901) av
...
!              '   standard deviation  = ',f12.5,/)
      ...
      ai,fracreject,absreject,abo,cosang,sinang,distint,delrsum,delcsum,iarray)
...
!-----------------------------------------------------------------------
      ...
      real*8    dtemp                                               !jd
      real      cosang,sinang,distint,fracreject,absreject          !jd
      real*8    delrsum(-1:n_ncol+1),delcsum(-1:n_nrow+1)           !jd
      integer   iarray(n_ncol,n_nrow)                               !jd
      integer   nums(MAXSAM)
      logical   first
...
!
            ...
            do 6 id=1,nd
                  ...
                  dy = y(id) - yloc
                  h2 = dx*dx + dy*dy
! Here we do the correction for prevention of crossing inactive areas.
                  if(ai.eq.'y')then
                    call fracinactive(n_ncol,n_nrow,x(id),y(id),xloc,yloc,cosang,sinang,   &
                    distint,delrsum,delcsum,iarray,fracreject,absreject,abo,ifrac)
                    if(ifrac.eq.1) h2=huge(h2)
                  end if
...
!
      ...
      subroutine fracinactive(ncol,nrow,x1,y1,x2,y2,cosang,sinang,distint,delrsum,delcsum,  &
                              iarray,fracreject,absreject,abo,ifrac)


! -- Subroutine FRACINACTIVE notionally draws a line between two points. In an
!    approximate way it determines the fraction of the length of the line that is in
!    a zone with an integer value of zero, or outside the grid altogether.

!    It is assumed that point coordinates are expressed in a coordinate system that is
!    zero at the top of left of the grid, but for which the row direction is
!    that of the true grid.

      implicit none

      integer, intent(in) :: ncol,nrow
      integer, intent(in) :: iarray(ncol,nrow)
      real,    intent(in) :: x1,x2,y1,y2
      real, intent(in)    :: cosang,sinang,distint,fracreject,absreject
      double precision, intent(in) :: delrsum(-1:ncol+1),delcsum(-1:nrow+1)
      character*1, intent(in) :: abo
      integer, intent(out):: ifrac


      integer          :: col,row,col1,row1,col2,row2,rowinc,colinc
      integer          :: fracount
      integer          :: absflag,fracflag
      double precision :: x1r,y1r,x2r,y2r,xtemp,ytemp,distot,cosa,sina,   &
                          xdistint,ydistint,fracdist
      double precision :: dist,xdist,ydist,disttot,fracadd

! -- Both point coordinates are transformed to coordinates in which the grid row
!    direction is assumed to be oriented east.

      x1r= x1*cosang+y1*sinang
      y1r=-x1*sinang+y1*cosang
      x2r= x2*cosang+y2*sinang
      y2r=-x2*sinang+y2*cosang

! -- Initialization

      ifrac=0

! -- Switching is done if necessary.
...
! -- Find the row and column in which each of these lie.

      do col=0,ncol+1
        if(x1r.lt.delrsum(col))then
          col1=col
          go to 10
        end if
      end do
...
! -- Direction of one point with respect to the other is now evaluated.

      if(row2.gt.row1)then
        rowinc=1
      else if (row2.lt.row1)then
        rowinc=-1
      else
        rowinc=0
      end if
      if(col2.gt.col1)then
        colinc=1
      else
        colinc=0
      end if

! -- The line is now traversed using the pre-defined increment.

      disttot=sqrt((x2r-x1r)*(x2r-x1r)+(y2r-y1r)*(y2r-y1r))
      if(disttot.le.0.0d0)then
        if((row1.eq.0).or.(row1.eq.nrow+1).or.                 &
           (col1.eq.0).or.(col1.eq.ncol+1))then
           ifrac=1
        else
           ifrac=1
           if(iarray(col1,row1).ne.0) ifrac=0
        end if
        go to 210
      else
        sina=(y2r-y1r)/disttot
        cosa=(x2r-x1r)/disttot
        xdistint=distint*cosa
        ydistint=distint*sina
        dist=0.0d0
        xdist=x1r
        ydist=y1r
        fracount=0
...
50      continue
        fracadd=1
        if((row.ge.1).and.(row.le.nrow))then
          if((col.ge.1).and.(col.le.ncol))then
            if(iarray(col,row).ne.0)fracadd=0
          end if
        end if
        if(fracadd.eq.0)then
          fracount=0
        else
          fracount=fracount+fracadd
          fracdist=(fracount-1)*distint
          fracflag=0
          absflag=0
          if(fracdist/disttot.gt.fracreject)then
            if(abo.eq.'o')then
              ifrac=1
              go to 210
            end if
          ...
          if(abo.eq.'b')then
            if((fracflag.eq.1).and.(absflag.eq.1))then
              ifrac=1
              go to 210
            end if
          end if
        ...
        dist=dist+distint
        if(dist.gt.disttot) go to 210
        xdist=xdist+xdistint
        ydist=ydist+ydistint
        if(xdist.ge.delrsum(col)) col=col+1
        if(rowinc.eq.1)then
          if(ydist.le.delcsum(row))row=row+1
        else if (rowinc.eq.-1)then
          if(ydist.gt.delcsum(row-1))row=row-1
        end if
        go to 50
      end if
