...
! -- Program PESTPREP1 prepares a PEST control file and a PEST instruction
...
!    prepared on a preceding MOD2OBS or SMP2SMP run.
	...
	write(amessage,5)
5       format(' Program PESTPREP1 prepares a PEST control file and ', &
...
580	        format(' Too many parameters - increase MAXPAR ',  &
	        'and re-compile program PESTPREP1.')
	        go to 9890
	      end if
	      apar(npar)=aapar
	    else
...
770	format(' 10.0  2.0  0.3  0.03  10  999')
...
890	format('* observation groups')
	write(outunit,900,err=9500) trim(agroup)
900	format(' ',a)
...
910	format('* observation data')
	...
	do i=1,nbore
	  do j=1,numsamp(i)
	    ...
	    call casetrans(aobs,'lo')
            write(outunit,920,err=9500) trim(aobs),value(jindex),trim(agroup)
920	    format(1x,a,t23,2x,1pg14.7,2x,'1.0  ',a)
          end do
        end do
