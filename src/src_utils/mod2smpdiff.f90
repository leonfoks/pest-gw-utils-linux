...
! -- Program MOD2SMPDIFF writes a bore sample file of model-generated head differences
!    time, interpolated to the sites of user-supplied bores.
	...
	character (len=1)   :: af,at,ax,ard
	character (len=15)  :: adate,atime,anum1,anum2
	character (len=16)  :: text


	write(amessage,5)
5       format(' Program MOD2SMPDIFF writes a bore sample file of model-generated ',&
	'head/concentration differences or ratios over time, interpolated to the sites of user-supplied bores.')
...
100     call read_bore_diff_file(ifail, &
       ' Enter name of bore difference listing file: ')
	if(ifail.ne.0) go to 9900
	if(escset.ne.0) then
	  escset=0
	  write(6,*)
...
120     continue
        ard=' '
        write(6,122,advance='no')
122     format(' Take ratio or difference of first to second column of this file?  [r/d]: ')
        read(5,'(a)') ard
        if(ard.eq.' ') then
          go to 120
        else if((ard.eq.'e').or.(ard.eq.'E'))then
          write(6,*)
          go to 100
        else if((ard.eq.'R').or.(ard.eq.'r'))then
          ard='r'
        else if((ard.eq.'D').or.(ard.eq.'d'))then
          ard='d'
        else
          go to 120
        end if
...
130	  format(' Too many pairs cited in bore difference listing file ',a, &
...
140	  format(' No coordinates for bore ',a,' from bore difference listing ',&
...
145	  format(' None of the bores cited in bore difference listing file ',a, &
...
410	call open_output_file(ifail, &
	...
	if(ierr.ne.0) then
	  write(6,480)
480	  format(' Insufficient available memory to continue MOD2SMPDIFF ',&
...
1050	format(' - ',a,' arrays, covering ',a,' different model output ',&
	...
	do i=1,num_bore_list/2
          ii=num_bore_list/2+i
	  do itime=1,ntime
	    call elapsdate(total_time(itime),day_convert,dds,mms,yys,hhhs,&
	    mmms,ssss,ddf,mmf,yyf,hhhf,mmmf,sssf)
	    if((icellno(i).eq.-999).or.(icellno(ii).eq.-999)) boreinfo(i,itime)=7.1e37
	    if((boreinfo(i,itime).gt.1.0e37).or.(boreinfo(ii,itime).gt.1.0e37)) then
	      ax='x'
              boreinfo(i,itime)=1.1e37
	    else
	      ax=' '
              if(ard.eq.'d')then
                boreinfo(i,itime)=boreinfo(i,itime)-boreinfo(ii,itime)
              else
                if(boreinfo(ii,itime).eq.0.0)then
                  if(boreinfo(i,itime).eq.0.0)then
                    boreinfo(i,itime)=1.0
                  else
                    if(boreinfo(i,itime).gt.0.0)then
                      boreinfo(i,itime)=1.0e30
                    else
                      boreinfo(i,itime)=-1.0e30
                    end if
                  end if
                else
                  dtemp=boreinfo(i,itime)/boreinfo(ii,itime)
                  if(dtemp.gt.1.0d30)then
                    boreinfo(i,itime)=1.0e30
                  else if(dtemp.lt.-1.0d30)then
                    boreinfo(i,itime)=-1.0e30
                  else
                    boreinfo(i,itime)=dtemp
                  end if
                end if
              end if
            end if
	    if(datespec.eq.1) then
	      write(outunit,1100,err=9400) trim(bore_diff_id(i)), ddf,mmf,yyf,&
	      hhhf,mmmf,sssf,boreinfo(i,itime),ax
	    else
	      write(outunit,1100,err=9400) trim(bore_diff_id(i)), mmf,ddf,yyf,&
	      hhhf,mmmf,sssf,boreinfo(i,itime),ax
	    end if
