...
! -- Program ELEV2CONC prepares initial concentrations for a SEAWAT model based on an array of elevations of the
!    freshwater/saltwater interface.
       ...
       character*200 :: specfile,actbase,infile,elevbase,saltfile,  &
                        afile,outbase,uoutfile,outfile,concbase,bfile
       ...
       write(amessage,1)
1      format(' Program ELEV2CONC writes MT3D and/or SEAWAT initial concentration ', &
...
560    format(' Enter filename base for formatted output files: ')
       read(5,'(a)') afile
       atemp=afile(1:2)
       call casetrans(atemp,'lo')
       if(atemp(1:2).eq.'e ')then
...
570    aprompt = ' Enter name for unformatted output file: '
