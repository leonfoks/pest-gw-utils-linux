!     Last change:  JD   16 Dec 2000    4:18 pm
program zone2bln

! -- Program zone2bln produces a SURFER blanking file of the zonation of the
!    finite-difference grid.
	...
	write(amessage,5)
5       format(' Program ZONE2BLN produces a SURFER blanking file of the ',&
...
10      call spec_open(ifail,gridspec)
	...
	if(ierr.ne.0) then
	  write(amessage,50)
50        format(' Cannot allocate sufficient memory to run ZONE2BLN')
	  call write_message(leadspace='yes',endspace='yes')
	  go to 9900
	end if
...
100     format(/,' Working.....',/)
	...
	call zonelines_bln(outunit,ncol,nrow,gridspec%east_corner,&
...
!       September, 1995: modified for inclusion in Groundwater Data Utilities.
   ...
   40 NE=I-1
      ...
      SUBROUTINE ZONELINES_bln(iunit,NCOL,NROW,E0,N0,ECG,NCG,IARRAY,JARRAY,E,N,&
      MAXVERT,gridspec)

! -- Subroutine zonelines_bln writes integer array zone boundaries in SURFER
!    "blanking" format.
...
210       format(' Internal error in subroutine ZONELINES: increase value ', &
	  'assigned to parameter MAXVERT in zone2bln source code and recompile.')
	  call write_message(endspace='yes')
	  stop
	END IF
	...
	IF(((JCOL.EQ.ICOL).AND.(JROW.EQ.IROW)).AND.(NDIR.EQ.2)) THEN
	  WRITE(iunit,220) NUMVERT,0
  220     FORMAT(I5,1X,I5)
	  DO 250 I=1,NUMVERT
  250     CALL OWRITE(iunit,DBLE(E(I))+E0,DBLE(N(I))+N0)
	ELSE
	  GO TO 200
	END IF
  499 CONTINUE
