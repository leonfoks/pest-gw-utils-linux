...
! -- Program PPK2FACG performs the first step in kriging from a set of
!    pilot points to a general set of grid or mesh nodes; it calculates the
...
!    the value at each node.
        ...
        character (len=200)                            :: intfile,structfile, &
        ...
        write(amessage,5)
5       format(' Program PPK2FACG calculates point-to-node factors by which kriging ',&
        'is undertaken from a set of pilot points to an arbitrary set of grid nodes.')
...
!        end if
        ...
        write(6,*)
75      aprompt=' Enter name of nodal x,y,zone file: '
...
! -- The node file is pre-read in order to ascertain the number of nodes in the mesh.

        maxnode=0
        do
          maxnode=maxnode+1
          read(labunit,*,err=9100,end=81)
        end do
81      continue
        rewind(unit=labunit)

        allocate(east(maxnode),north(maxnode),eastgrid(maxnode),northgrid(maxnode), &
        icellno(maxnode),intarray(maxnode),stat=ierr)
        if(ierr.ne.0) then
          write(amessage,50)
...
! -- Grid coordinates and zone numbers are read from the nodal x,y,zone file. Note that
! -- the coordinates of the first grid node are subtracted from all mesh coordinates.
        ...
        currint=-99999
        jline=jline+1
52      read(labunit,'(a)',err=9100) cline
        ...
        do
          jline=jline+1
53        read(labunit,'(a)',err=9100,end=85) cline
          ...
          if(ifail.ne.0) go to 9550
          intarray(nummesh)=char2int(ifail,3)
          if(ifail.ne.0) go to 9550
          east(nummesh)=etemp-east1
          north(nummesh)=ntemp-north1
        end do
85      continue
        close(unit=labunit)
...
150         format(' The x,y,zone file can hold a maximum of ',a, &
            ' different integers for proper PPK2FACG execution. There are ', &
            ' more integers than this in file ',a,'. To increase ' &
            'this limit, edit PPK2FACG source code, increase parameter ', &
...
200     write(6,*)
205     write(6,210)
210     format(' The following zones have been detected in x,y,zone file:-')
        i=0
255     i=i+1
...
350     continue
        ...
        write(amessage,352)
352     format(' Interpolation must take place for at least one zone in nodal x,y,zone ', &
        'file - try again.')
        call write_message(leadspace='yes',endspace='yes')
        go to 200
351     continue
...
399         format(/,' Warning: no pilot points assigned to grid/mesh zone ',a)
            go to 410
          end if
          call num2char(intval(izone),anum1)
...
9110    format(' Error encountered while reading nodal x,y,zone file ',a)
        go to 9890
