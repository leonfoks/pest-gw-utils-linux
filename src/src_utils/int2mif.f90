!     Last change:  JD   16 Dec 2000    6:10 pm
program int2mif

! -- Program int2mif writes MAPINFO "MIF" and "MID" files of the finite-
!    difference grid based on the data contained in an integer array and on
!    the contents of a window integer array.
	...
	integer, allocatable, dimension(:,:)    :: intarraydat,intarraywin
	real, allocatable, dimension(:,:)       :: east,north
	character (len=80)                      :: aprompt,outfile1,outfile2


	write(amessage,5)
5       format(' Program INT2MIF writes MAPINFO "MIF" and "MID" files ', &
	'of the finite difference grid based on the data contained in an ',&
	'integer array and on the contents of a window integer array.')
...
10      call spec_open(ifail,gridspec)
	...
	ncol=gridspec%ncol
	nrow=gridspec%nrow
	allocate(intarraydat(ncol,nrow),intarraywin(ncol,nrow), &
	east(ncol,nrow),north(ncol,nrow),stat=ierr)
	if(ierr.ne.0) then
	  write(amessage,50)
50        format(' Cannot allocate sufficient memory to run INT2MIF.')
	  call write_message(leadspace='yes',endspace='yes')
	  go to 9900
	end if
...
52	write(6,*)
	aprompt=' Enter name of data integer array file: '
	call read_integer_array(ifail,aprompt,intarraydat,pm_header=headerspec, &
        rows=nrow, columns=ncol)
	if(ifail.ne.0) go to 9900
	if(escset.eq.1) then
	  escset=0
	  write(6,*)
	  deallocate(intarraydat,intarraywin,east,north,stat=ierr)
	  if(ierr.ne.0) then
	    write(amessage,55)
...
60	aprompt=' Enter name of window integer array file: '
	call read_integer_array(ifail,aprompt,intarraywin,pm_header=headerspec, &
        rows=nrow, columns=ncol)
	if(ifail.ne.0) go to 9900
	if(escset.eq.1) then
	  escset=0
...
100     format(/,' Working.....',/)
	...
	call write_mif_file(ifail,outunit1,outfile1,intarraywin,east,north, &
	gridspec,izone,'i')
	if(ifail.ne.0) go to 9900
...
150     format('  - "MIF" file ',a,' written ok.')
	...
	call write_integer_mid_file(ifail,outunit2,outfile2,intarraywin, &
	intarraydat)
	if(ifail.ne.0) go to 9900
